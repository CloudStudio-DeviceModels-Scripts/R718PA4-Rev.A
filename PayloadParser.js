...
function parseUplink(device, payload) {
    ...
    if (decoded.h2s != null) {
        ...
        if (sensor2 != null)
            sensor2.updatePpmConcentrationSensorStatus(decoded.h2s);
    };
}
...
function Decoder(bytes, fport) {
    ...
    if (fport === 6) { // then its ReportDataCmd
        if (bytes[2] === 0x00) { // version report
            ...
            return decoded;
        } else if ((bytes[0] === 0x01) && (bytes[1] === 0x57) && (bytes[2] === 0x06)) { // device type 57 (R718PA4)
            decoded.devicetype = "R718PA4";
